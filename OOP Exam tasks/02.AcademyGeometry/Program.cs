...
namespace GeometryAPI
{
    ...
    public class AdvancedFigureController : FigureController
    {
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "normal":
                    {
                        IFlat flattened = this.currentFigure as IFlat;
                        if (flattened != null)
                        {
                            Console.WriteLine("{0:0.00}", flattened.GetNormal());
                        }
                        else
                        {
                ...
                case "area":
                    {
                        IAreaMeasurable abstracted = this.currentFigure as IAreaMeasurable;
                        if (abstracted != null)
                        {
                            Console.WriteLine("{0:0.00}", abstracted.GetArea());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "volume":
                    {
                        IVolumeMeasurable abstracted = this.currentFigure as IVolumeMeasurable;
                        if (abstracted != null)
                        {
                            Console.WriteLine("{0:0.00}", abstracted.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        public Cylinder(Vector3D bottomCenter, Vector3D upperCenter, double radius)
            : base(bottomCenter, upperCenter)
        {
            this.Radius = radius;
        }
        ...
        public double GetArea()
        {
            ...
            return 2 * Math.PI * this.Radius * this.Radius + 2 * Math.PI * this.Radius * height;
        }
